program bingo2
  ...
  ! Now, we want the last winning board
  ...
  integer(kind=4), allocatable :: numbers(:), sets(:,:,:), last(:)
  ...
  nsets = size(sets,1)
  allocate(marks(nsets, 5, 5), last(1:nsets))
  marks = .false.
  last = 0
  
  ! We operate array by array, and keep track of the last digit
  do i=1, nsets
     do d=1,ndigits
  ...
  win = maxloc(last,1)
  lastnum = numbers(last(win))
  write(*,'(A4,I2,A28,I2)') 'Set', win, 'has won last after calling ', lastnum
